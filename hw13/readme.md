...
Сначала посмотрим, что у нас на несекционированной таблице (результат примерно одинаков, для повторных запросов):
...
QUERY PLAN                                                                                                                            |
--------------------------------------------------------------------------------------------------------------------------------------+
Index Scan using boarding_passes_pkey on boarding_passes  (cost=0.56..16.61 rows=3 width=25) (actual time=0.025..0.027 rows=2 loops=1)|
  Index Cond: (ticket_no = '0005433227218'::bpchar)                                                                                   |
Planning Time: 0.059 ms                                                                                                               |
Execution Time: 0.040 ms                                                                                                              |
```

А теперь посмотрим на результат в секционированной таблице:

```
explain analyze select * from bookings.boarding_passes_p where ticket_no = '0005433227218';

QUERY PLAN                                                                                                                                         |
---------------------------------------------------------------------------------------------------------------------------------------------------+
Gather  (cost=1000.00..27349.49 rows=7 width=25) (actual time=23.545..68.763 rows=4 loops=1)                                                       |
  Workers Planned: 2                                                                                                                               |
  Workers Launched: 2                                                                                                                              |
  ->  Parallel Seq Scan on boarding_passes_p_2 boarding_passes_p  (cost=0.00..26348.79 rows=3 width=25) (actual time=29.252..57.445 rows=1 loops=3)|
        Filter: (ticket_no = '0005433227218'::bpchar)                                                                                              |
        Rows Removed by Filter: 699185                                                                                                             |
Planning Time: 0.062 ms                                                                                                                            |
Execution Time: 68.777 ms                                                                                                                          |                       
```

Результат совсем плохой, попробуем добавить индекс для ключевого поля ticket_no:

```
CREATE INDEX ticket_no_pkey_1 ON ONLY bookings.boarding_passes_p USING btree (ticket_no)
```

Стало заметно лучше, но очевидного выигрыша во времени мы не получаем:
...
QUERY PLAN                                                                                                                              |
----------------------------------------------------------------------------------------------------------------------------------------+
Bitmap Heap Scan on boarding_passes_p_2 boarding_passes_p  (cost=4.48..32.12 rows=7 width=25) (actual time=0.021..0.025 rows=4 loops=1) |
  Recheck Cond: (ticket_no = '0005433227218'::bpchar)                                                                                   |
  Heap Blocks: exact=4                                                                                                                  |
  ->  Bitmap Index Scan on boarding_passes_p_2_ticket_no_idx  (cost=0.00..4.48 rows=7 width=0) (actual time=0.016..0.016 rows=4 loops=1)|
        Index Cond: (ticket_no = '0005433227218'::bpchar)                                                                               |
Planning Time: 0.070 ms                                                                                                                 |
Execution Time: 0.039 ms                                                                                                                |
```
Возможно такой неочевидный результат связан с тем, что все таки выбраны не лучшие данные для организации секционирования таблицы
...
Тестирование обращений к секционированной таблице уже проводили. Попробуем добавить какие нибудь данные.

```
demo=# INSERT INTO bookings.boarding_passes_p VALUES ('0005432000111', 123123, 666, '18A');
INSERT 0 1
```

И запросим эти данные:

```
select * from bookings.boarding_passes_p where ticket_no = '0005432000111';

ticket_no    |flight_id|boarding_no|seat_no|
-------------+---------+-----------+-------+
0005432000111|   123123|        666|18A    |
```

```
explain analyze select * from bookings.boarding_passes_p where ticket_no = '0005432000111';


QUERY PLAN                                                                                                                              |
----------------------------------------------------------------------------------------------------------------------------------------+
Bitmap Heap Scan on boarding_passes_p_0 boarding_passes_p  (cost=4.47..28.12 rows=6 width=25) (actual time=0.021..0.022 rows=1 loops=1) |
  Recheck Cond: (ticket_no = '0005432000111'::bpchar)                                                                                   |
  Heap Blocks: exact=1                                                                                                                  |
  ->  Bitmap Index Scan on boarding_passes_p_0_ticket_no_idx  (cost=0.00..4.47 rows=6 width=0) (actual time=0.016..0.016 rows=1 loops=1)|
        Index Cond: (ticket_no = '0005432000111'::bpchar)                                                                               |
Planning Time: 0.080 ms                                                                                                                 |
Execution Time: 0.037 ms     
```
Данные попали в секцию boarding_passes_p_0 как и должны были и были успешно получены.
